...
export const MapboxProvider = ({ children }) => {
  ...
  const deckOverlay = useRef(null); // Ref for Deck.gl overlay    const tooltipRef = useRef(null); 
  
  useEffect(() => {
    if (map.current) return;
    
    const mapboxStyleUrl = 'mapbox://styles/mapbox/satellite-streets-v12'; 
    
    mapboxgl.accessToken = accessToken;
    map.current =  new mapboxgl.Map({
      container: mapContainer.current,
      style: mapboxStyleUrl,
      center: [-82.771556, 32.967243], // Centered on the US
      zoom: 6,
      options: {
        trackResize: true,
      },
